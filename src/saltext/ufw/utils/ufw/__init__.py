...
def rules_match(x, y):  # pylint: disable=too-many-return-statements
    ...
    """
    if x.dport != y.dport:
        log.debug("Dport mismatch: %s != %s", x.dport, y.dport)
        return 1
    if x.sport != y.sport:
        log.debug("Sport mismatch: %s != %s", x.sport, y.sport)
        return 1
    if x.protocol != y.protocol:
        log.debug("Protocol mismatch: %s != %s", x.protocol, y.protocol)
        return 1
    if x.dst != y.dst:
        log.debug("Dst mismatch: %s != %s", x.dst, y.dst)
        return 1
    if x.src != y.src:
        log.debug("Src mismatch: %s != %s", x.src, y.src)
        return 1
    if x.dapp != y.dapp:
        log.debug("Dapp mismatch: %s != %s", x.dapp, y.dapp)
        return 1
    if x.sapp != y.sapp:
        log.debug("Sapp mismatch: %s != %s", x.sapp, y.sapp)
        return 1
    if x.interface_in != y.interface_in:
        log.debug("Interface_in mismatch: %s != %s", x.interface_in, y.interface_in)
        return 1
    if x.interface_out != y.interface_out:
        log.debug("Interface_out mismatch: %s != %s", x.interface_out, y.interface_out)
        return 1
    if x.direction != y.direction:
        log.debug("Direction mismatch: %s != %s", x.direction, y.direction)
        return 1
    if x.forward != y.forward:
        log.debug("Forward mismatch: %s != %s", x.forward, y.forward)
        return 1
